...
export async function POST(req: NextRequest) {
  ...
  await db.collection('notifications').insertOne({ title, body, deleted: false });
...
export async function DELETE(req: NextRequest) {
  ...
  const db = await getMongoClient();
  const notification = await db.collection('notifications').findOne({ _id: new ObjectId(String(_id)) });
  if (!notification) {
    return NextResponse.json(
