        ...
        <CardContent className="p-8 md:p-12">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
            ...
            <div className="space-y-4">
              ...
              <Textarea
                ...
                className={`min-h-[140px] bg-white/50 backdrop-blur-sm border-2 transition-all duration-300 resize-none text-gray-700 placeholder:text-gray-400 ${
                  focusedField === "quote" ? "border-blue-400 shadow-lg shadow-blue-400/20 bg-white/80" : "border-gray-200/50 hover:border-gray-300/70"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Name */}
              <InputWithLabel id="name" label="Your Name" icon={<User />} error={errors.name} fieldProps={nameProps} setFocusedField={setFocusedField} focusedField={focusedField} />

              {/* Location */}
              <InputWithLabel id="location" label="Location" icon={<MapPin />} error={errors.location} fieldProps={locationProps} setFocusedField={setFocusedField} focusedField={focusedField} />

              {/* Company */}
              <InputWithLabel id="company" label="Company (optional)" icon={<Building2 />} fieldProps={companyProps} setFocusedField={setFocusedField} focusedField={focusedField} />

              {/* Username */}
              <div className="space-y-3">
                <Label htmlFor="username" className="text-base font-medium text-gray-700 flex items-center space-x-2">
                  <Instagram className="h-4 w-4 text-gray-500" />
                  <span>Instagram <span className="text-gray-400 text-sm">(optional)</span></span>
                </Label>
                <div className="flex">
                  <span className="inline-flex items-center px-4 rounded-l-lg border-2 border-r-0 border-gray-200/50 bg-gray-50/50 text-gray-500 text-sm font-medium">@</span>
                  <Input
                    id="username"
                    className={`rounded-l-none bg-white/50 backdrop-blur-sm border-2 transition-all duration-300 ${
                      focusedField === "username" ? "border-blue-400 shadow-lg shadow-blue-400/20 bg-white/80" : "border-gray-200/50 hover:border-gray-300/70"
                    }`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 bg-gradient-to-br from-gray-50/50 to-blue-50/50 rounded-2xl border border-white/30">
              <InputWithLabel id="logo" label="Company Logo Name (optional)" icon={<Sparkles />} fieldProps={logoProps} setFocusedField={setFocusedField} focusedField={focusedField} />
              <div className="space-y-3">
                <Label htmlFor="logoColor" className="text-base font-medium text-gray-700 flex items-center space-x-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {colorOptions.map((color) => (
                    <label key={color.value} className="cursor-pointer">
                      ...
                      <div className={`w-full h-12 ${color.color} rounded-lg border-2 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105 flex items-center justify-center
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-gray-700">
            Thank you for sharing your experience with us.<br />
            <span className="mt-2 block text-sm text-gray-500">Your feedback will be reviewed and showcased on our homepage.</span>
    ...
    <div className="space-y-3">
      ...
      <Input
        ...
        className={`bg-white/50 backdrop-blur-sm border-2 transition-all duration-300 ${
          focusedField === id ? "border-blue-400 shadow-lg shadow-blue-400/20 bg-white/80" : "border-gray-200/50 hover:border-gray-300/70"
